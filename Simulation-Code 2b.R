...
#########################################################################################################################
#####
##### Code 2b. Evaluate the accuracy of QTL detection using different statistics ----
#####
########################################################################################################################
...
## c - Define data frames where to save the results of multiple simulations 
## d - Load the function for LOD and SmLOD calculation before starting the loop
## e - Load DP data from rice sequencing dataset (Cubry et al 2018)
...
## b - Code parameters ----
test<-10      # Number of statistical tests  of statistical methods evaluated (considering single marker based and window based statistics) 
nbsim<-1000  # Number of simulations 1000
...
## c - Data frames to save output ----
QTLpos<- as.data.frame(matrix(ncol = test, 
                              nrow = nbsim, 
                              dimnames = list(c(1:nbsim), 
                                              c("P0","PdeltaSNP","PTdeltaSNP",
                                                "PG", "PGprime",
                                                "PEDm","PED100_4",
                                                "PLOD", "PSmLOD",
                                                "PAFDexp"))))

## d - Load the LOD function from Zhang et al. 2019 obtained from:
##     https://github.com/caulilin/QTG_Seq/tree/master/SmoothLOD
...
## e - Load DP data from rice sequencing dataset (Cubry et al 2018)
library(vcfR)
vcf <- read.vcfR( "./OgOb-all-merged.DPech.recode.vcf", verbose = TRUE )
...
for (n in 1:nbsim){ 
  ...
  # 1.3 QTL and Bulks----
  qtlpos<-sample(loci,1) # random position for a simulated QTL
  QTLpos$P0[n]<-qtlpos #save the initial QTL position
  ...
  #Add sequencing noise to the data using real sequencing data from rice from Cubry et al. 2018 (DP_rice.csv)
  ...
  for (i in 1:nrow(simdata)){
    ...
    freq<-simdata$AD_ALT.LOW_i[i]/(simdata$AD_ALT.LOW_i[i]+simdata$AD_REF.LOW_i[i])
    
    position<-rbinom(100,1,freq)
    ...
    sum1=sum(DP[i,1:100])-sum0
    
    simdata$AD_ALT.LOW[i]<-round(sum0/mean(DP[,1:100]))
    
    simdata$AD_REF.LOW[i]<-round(sum1/mean(DP[,1:100]))
    
    freq<-simdata$AD_ALT.HIGH_i[i]/(simdata$AD_ALT.HIGH_i[i]+simdata$AD_REF.HIGH_i[i])
    ...
    simdata$AD_ALT.HIGH[i]<-round(sum0/mean(DP[,1:100]))
    
    simdata$AD_REF.HIGH[i]<-round(sum1/mean(DP[,1:100]))
  ...
  #Save QTL positions retrieved by the four statistics calculated through QTLseqr
  QTLpos$PdeltaSNP[n]<-mean(simdata[abs(simdata$deltaSNP)==max(abs(simdata$deltaSNP)),c(2)])
  QTLpos$PTdeltaSNP[n]<-mean(simdata[abs(simdata$tricubeDeltaSNP)==max(abs(simdata$tricubeDeltaSNP)),c(2)])
  QTLpos$PG[n]<-mean(simdata[simdata$G==max(abs(simdata$G)),c(2)])
  QTLpos$PGprime[n]<-mean(simdata[simdata$Gprime==max(abs(simdata$Gprime)),c(2)])
  ...
  #Save QTL positions retrieved by the two ED statistics:
  QTLpos$PEDm[n]<-mean(simdata[simdata$EDm==max(abs(simdata$EDm)),c(2)])
  QTLpos$PED100_4[n]<-mean(simdata[simdata$ED100_4==max(abs(simdata$ED100_4)),c(2)])
  ...
  ## b - Run the LOD function from Zhang et al. 2019 
  ...
  ## b - Run LODFunction.R from Zhang et al. 2019 ----
  # 
  ...
  #Save QTL positions retrieved by LOD and Smooth LOD statistics
  QTLpos$PLOD[n]<-mean(LODresult[LODresult$LOD==max(abs(LODresult$LOD)),c(2)])
  QTLpos$PSmLOD[n]<-mean(LODresult[LODresult$Smooth_LOD==max(abs(LODresult$Smooth_LOD)),c(2)])
  ...
  #Save QTL positions identified by the BRM method:
  QTLpos$PAFDexp[n]<-result2[1,2]
  
  #The QTLpos matrix contains the QTL position retrieved with each statistic in nbsim simulations
  #We need to take into account 10 kb between markers to translate marker position into genetic distance. 
  
}

write.csv(QTLpos,file = "QTLpos.csv",row.names = FALSE)
